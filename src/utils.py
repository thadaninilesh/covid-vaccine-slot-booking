...
PINCODE_URL = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/findByPin?pincode={0}&date={1}"
...
def check_calendar(request_header, vaccine_type, minimum_slots, min_age_booking, pincodes):
    """
    This function
    ...
    try:
        ...
        for pincode in pincodes:
	        ...
	        base_url = PINCODE_URL

	        options = []
	        request_url = base_url.format(pincode, tomorrow)
	        resp = requests.get(request_url, headers=request_header)
...
def check_and_book(request_header, beneficiary_dtls, minimum_slots, pincodes):
    """
    This function
    ...
    try:
        ...
        options = check_calendar(request_header, vaccine_type, minimum_slots, min_age_booking, pincodes)
        ...
        if len(tmp_options) > 0:
            beep(WARNING_BEEP_DURATION[0], 2000)
